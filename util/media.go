...
// PatientPhotoPath returns the absolute file path for a patient's visit photo.
// We store the image as a file named by the visit id (no extension) under uploads/patient/<id>/.
// MIME is derived at read time.
func PatientPhotoPath(id int32, vid int32) string {
    base := filepath.Join(MediaRoot(), "patient", fmt.Sprintf("%d", id))
    return filepath.Join(base, fmt.Sprintf("%d", vid))
}
...
// SavePatientPhoto writes the bytes to the deterministic path for the patient visit
func SavePatientPhoto(id, vid int32, data []byte) error {
    abs := PatientPhotoPath(id, vid)
...
func DeletePatientPhotoIfExists(id, vid int32) error {
    abs := PatientPhotoPath(id, vid)
